 ...
 * page with a country + device breakdown. Both poll independently -- the
 * 10s server-side dedupe in fetchGA4Realtime keeps that from doubling the
 * GA4 API call rate when an admin has both pages open.
 */
      ...
      {error ? (
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-[auto_1fr]">
          ...
          {data && data.activeUsers > 0 && (
            <div className="flex flex-wrap gap-x-8 gap-y-3 border-t border-white/[0.06] pt-4 sm:border-t-0 sm:border-l sm:pl-6 sm:pt-0">
                    ...
                    {data.byCountry.slice(0, 5).map((c) => (
                      <li key={c.country} className="flex items-center justify-between gap-4 text-[12.5px] text-white/65">
                        <span className="truncate">
                          {[flagEmoji(c.code), c.country].filter(Boolean).join(" ")}
                        </span>
