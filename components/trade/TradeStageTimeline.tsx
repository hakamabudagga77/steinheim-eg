...
export default function TradeStageTimeline({ status }: { status: TradeLeadStatus }) {
  const t = useTranslations("tradeStageTimeline");

  if (status === "lost") {
    return (
      <div className="border border-charcoal/10 bg-white p-4">
        <p className="text-[9px] font-medium uppercase tracking-[0.2em] text-warm-gray">{t("projectStatus")}</p>
  ...
  return (
    <div className="border border-charcoal/10 bg-white p-4">
      <div className="flex items-center justify-between">
        <p className="text-[9px] font-medium uppercase tracking-[0.2em] text-warm-gray">{t("projectJourney")}</p>
        <p className="text-[9px] font-medium uppercase tracking-[0.1em] text-warm-gray/70">
          {t("stepOf", { current: currentIndex + 1, total })}
        </p>
      </div>
