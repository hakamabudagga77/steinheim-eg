...
// The hero (eyebrow + h1) doesn't depend on the URL's ?items= param, so it's
// rendered unconditionally here rather than inside the Suspense boundary
// below - useSearchParams() forces whatever reads it to defer to the client,
// and a crawler that doesn't wait for that hydration would otherwise see a
...
function ItemCount() {
  const t = useTranslations("sharedWishlistPage");
  const entries = useWishlistEntries();
